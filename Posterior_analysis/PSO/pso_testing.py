...
from Posterior_analysis.PSO.pso_behaviour import plot_pso_chain
...
if __name__=="__main__":
    ...
    backup_path   = "backup_results/pso_test"
    savemcmc_path = get_savemcmcpath(setting_name,backup_path)#f"{get_savemcmcpath(setting_name,'').replace('./','')}"
    savefig_path  = get_savefigpath(setting_name, backup_path)#f"pso_test_{get_savefigpath(setting_name,'')}"
    ...
    for nPSO in range(num_PSO):
        ...
        savefig_path_i  = f"{savefig_path}/pso_it_{nPSO}/"
        savemcmc_path_i = f"{savemcmc_path}/pso_it_{nPSO}/"
        mkdir(savefig_path_i)
        mkdir(savemcmc_path_i)
        os.system("cp "+setting_path+"/"+setting_name+".py "+savefig_path_i+".") #we copy the setting file to that directory
        ...
        print_res = open(savefig_path_i+"results.txt","w")
        ...
        fitting_kwargs_list = [['MY_PSO', {'sigma_scale': 1., 'n_particles': n_particles, 
                                           'n_iterations': n_iterations,"path":savemcmc_path_i,"threadCount":threadCount}]]
        ...
        name_pso,chain_pso,param_pso=pso_chain
        name_json = save_json_name(setting,savemcmc_path_i,"pso")
        save_json(chain_pso,name_json)
        if i==0:
            name_param_json = save_json_name(setting,savemcmc_path,"pso_params")
            save_json(param_pso,name_param_json)
        
        kwargs_result   = fitting_seq.best_fit()
        
        f, axes = plt.subplots(1, 1, figsize=(18, 6))
        num_avrg = 100
        num_samples=len(chain_pso)
        if int(num_samples/num_avrg)>1000:
            num_avrg= num_samples/960
        try:
            #my_plot_mcmc_logL(axes,chain_pso,num_avrg)
            #del_ax = axes.plot([],[])
            #del_ax[0].set_color("r")
            #plt.savefig(savefig_path_i+'/MCMC_bhv_LogL.png')
            #plt.close()
            plot_pso_chain(pso_chain=pso_chain,setting=setting,savefig_path=savefig_path_i)
        except:
            print("plot_pso_chain still to get right")
        ...
        PM(modelPlot,savefig_path_i,v_min,v_max,res_min,res_max)

        pickle_results("read_results",kwargs_result,savefig_path_i)
        ...
        plt.savefig(savefig_path_i+"normalised_residuals.png")
        plt.close()
        ...
        plt.savefig(savefig_path_i+"caustics.png")
        plt.close()
        ...
        plt.savefig(savefig_path_i+"point_source_position.png")
        plt.close()
