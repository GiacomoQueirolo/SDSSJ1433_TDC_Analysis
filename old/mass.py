...
from conversion import conv_radec_to_xy,conv_total
...
class Grid_Class():
    ...
    def __init__(self,center_x,center_y,radius,precision=.01,type="pixel",setting=None):
        ...
        if self.type=="radec":
            self.radec_grid = self.get_transf_grid("to_radec") 
        else:
            self.radec_grid = self.grid
    
    def create_grid(self):
        n  = int(self.radius/self.precision)
        n  = max([1,n]) 
    ...
    def circularise_flat_grid_with_area(self):
        ...
        grid_area_flat = self.grid_area.reshape(len(self.grid_area)**2,2)
        grid = []
        area = []
        for i in range(len(grid_flat)):
            if np.sqrt(grid_flat[i][0]**2+grid_flat[i][1]**2)<=radius:
                grid.append(np.array(grid_flat[i]).tolist())
                area.append(np.array(grid_area_flat[i]).tolist())
        self.circ_flat      = np.array(grid)
        self.area_circ_flat = np.array(area)
        return self.circ_flat,self.area_circ_flat
    

    def get_transf_grid(self,trasnf="to_pixel"):
        if trasnf=="to_pixel" and self.pixel_grid:
            return self.pixel_grid
        elif trasnf=="to_radec" and self.radec_grid:
            return self.radec_grid
        grid_trnsf = np.ones_like(self.grid).tolist()
        for i in range(len(self.grid)):
            for j in range(len(self.grid[i])):
                if trasnf == "to_pixel":
                    grid_trnsf[i][j] = conv_radec_to_xy(self.setting,*self.grid[i][j])
                elif trasnf== "to_radec":
                    grid_trnsf[i][j] = conv_total(self.setting,*self.grid[i][j])
        return np.array(grid_trnsf)
    ...
    def convert_grid(self):
        ...
        if self.type=="radec":
            #convert to pixel
            self.type      = "pixel"
            self.center    = [conv_radec_to_xy(self.setting,*self.center)]
            ...
            #convert to radec
            self.type      = "radec"
            self.center    = [conv_xy_to_radec(self.setting,*self.center)]
            self.radius    = self.radius*pixtoarcsec
            self.precision = self.precision*pixtoarcsec
...
def get_grid_area(grid):
    ...
    if np.all(np.abs(dx-dx[0])<prec) and np.all(np.abs(dy-dy[0])<prec):
        area = dx[0]*dy[0]
        grid_area = np.ones(np.shape(grid)[:-1])*area
        return grid_area
    else:
        print(dx,dy)
        # pragma: no cover
...
def circularise_flat_grid_with_area(grid_flat,grid_area_flat,radius):
    grid = []
    area = []
    for i in range(len(grid_flat)):
        if np.sqrt(grid_flat[i][0]**2+grid_flat[i][1]**2)<=radius:
...
def get_lens_model(setting):
    ...
    lensModel = LensModel(lens_model_list=lens_model_list, z_lens=z_lens, z_source=z_source)
    return lensModel
"""   
...
def get_kappa_discreet(setting,radius,kwlens,lens_model,dr=0.01):
    center_lens_x,center_lens_y = kwlens[0]["center_x"],kwlens[0]["center_y"] 
    grid = Grid_Class(setting=setting,center_x=center_lens_x,center_y=center_lens_y,radius=radius,precision=dr,type="radec")
...
def get_mass_discreet(setting,radius,lens_model,kwlens,Sigma_crit,cosmo_dd,dr=0.01,ret_grid=False):
    ...
    kappa_discreet, grid =  get_kappa_discreet(setting,radius,kwlens,lens_model,dr=dr) 
