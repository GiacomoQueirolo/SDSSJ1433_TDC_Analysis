# A copy of lenstronomy.Sampling
import numpy as np
...
class PriorLikelihood_KDE(object):
    """
    ...
    def __init__(self, prior_lens=None, prior_source=None, prior_lens_light=None, prior_ps=None, prior_special=None,
        ...
        :param mod_qphi_prior: boolean to activate non-flat, "semigaussian" prior for qphi (irregular for e1,e2) 
        ...
        self.mod_qphi_prior = False
        if precomputed_kde is None:
            ...
            self._kde_lens_list       = precomputed_kde.lens_list
            self._kde_source_list     = precomputed_kde.source_list
            self._kde_lens_light_list = precomputed_kde.lens_light_list
            self._kde_ps_list         = precomputed_kde.ps_list
            self.mod_qphi_prior       = True
    ...
    def logL(self, kwargs_lens=None, kwargs_source=None, kwargs_lens_light=None, kwargs_ps=None, kwargs_special=None,
        ...
        if not self.mod_qphi_prior:
            logL += self._prior_kde_list(kwargs_lens, self._prior_lens_kde, self._kde_lens_list)
        else:
            logL += self._prior_kde_list_qphiprior(kwargs_lens, self._kde_lens_list)
    ...
    @staticmethod
    def _prior_kde_list_qphiprior(kwargs_lens_list,kde_lens):
        """
        ...
        :return:
        """
        if kde_lens == 0 or kde_lens is None:
            return 0
        logL = np.log(kde_lens.likelihood(kwargs_lens_list))
        return logL
    @staticmethod
