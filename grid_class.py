...
def dist(xy0,xy1):
    return np.sqrt( np.square(xy0[0]-xy1[0]) + np.square(xy0[1]-xy1[1]) ) 
...
class Square_Grid():
    ...
    def __init__(self,center,radius,edge,setting,input_type="radec",r_mask=None,cnt_mask=None):
        ...
        self.shape_grid   = np.shape(self.grid)
        if r_mask:
            self.mask()
        
    def _flatten(self,grid):
        try:
            return np.array(grid).reshape(np.shape(grid)[0]*np.shape(grid)[1],2).tolist()
        except ValueError: 
    ...
    def get_area_square(self,output_type="radec"):
        # get the area of the single Square (the "Pixel")
        verify_type(output_type)
    ...
    def get_area_square_grid(self,output_type="radec"):
        # get an array with the area for every pixel
        area_square = self.get_area_square(output_type)
        return area_square * np.ones(self.shape_grid)
    ...
    def plot_grid(self,color="green",ax=None,grid_to_plot="grid",alpha=.7):
        ...
        if grid_to_plot not in ["grid","degraded_pixel_grid"]:
            raise RuntimeError('Can only plot either "grid" or "degraded_pixel_grid"')
        else:
    ...
    def produce_degraded_fits(self,name="degraded_grid",verbose=True,_other_grid=None):
        ...
        for px in grid:
            # prob due to differences in array structure
            # this is the correct order of the indexes
            mask[px[1]][px[0]] = 1
    ...
    def get_subgrid(self,r,input_type="pixel",newCenter=None):
        ...
        if R<self.Radius:
            ...
            SubGrid.shape_grid  = np.shape(SubGrid.grid)
            if hasattr(self,"degraded_pixel_grid"):
...
class Circ_Grid(Square_Grid):
    ...
    def __init__(self,center,radius,edge,setting,input_type="radec",r_mask=None,cnt_mask=None):
        ...
        self.shape_circ_grid = np.shape(self.circ_grid)
        if r_mask:
            self.mask()
    ...
    def mask(self,list_points=None,input_type="radec"):
        if list_points is None:
            ...
            return super(Circ_Grid, self).mask(list_points,input_type=input_type)
    
    def _get_degraded_pixel_circ_grid(self):
        sqrt_deg_pix = super(Circ_Grid, self).get_degraded_pixel_grid()
    ...
    def get_area_circ_grid(self,output_type="radec"):
        area_square = self.get_area_square(output_type)
        return area_square * np.ones(self.shape_circ_grid)
    """
    def get_totarea_circ_grid(self,output_type="radec"):
        verify_type(output_type)
        # still the same area for each square
        return self.get_area_square(output_type)*len(self.get_circ_grid())
    
    def plot_grid(self,color="green",ax=None,grid_to_plot="circ_grid",alpha=.7):
        if not ax:
            fig,ax=plt.subplots()
        if grid_to_plot not in ["circ_grid","degraded_pixel_circ_grid"]:
            try:
                return super(Circ_Grid, self).plot_grid(color=color,ax=ax,grid_to_plot=grid_to_plot,alpha=alpha)
            except RuntimeError:
                raise RuntimeError('Can only plot either "circ_grid" or "degraded_pixel_circ_grid"')
        else:
    ...
    def produce_degraded_fits(self,name="degraded_circ_grid",verbose=True):
        super(Circ_Grid,self).produce_degraded_fits(name,verbose,_other_grid=self.get_degraded_pixel_circ_grid())
    
    def get_subgrid(self,r,input_type="pixel",newCenter=None):
        if not isinstance(r,Length):
            R = Length(r,setting=self.setting,input_type=input_type)
        else:
            R=r
        if R==self.Radius:
        ...
        if R<self.Radius:
            ...
            SubGrid.shape_circ_grid = np.shape(SubGrid.circ_grid)
            if hasattr(self,"degraded_pixel_circ_grid"):
                #this is quite slow actually
                subgrid_degraded_select = [ dist(gi,newCenter.get_pixel())<R.length_pix for gi in self.degraded_pixel_circ_grid]
...
def _plot_grid_on_image(grid,image=None,savename=None):
    if not isinstance(grid,list):
        grid = [grid]
    sett = grid[0].setting # assuming all grids have the same setting
    fig,ax = plt.subplots(1)
    if image is None:
        image = load_fits(sett.data_path+"/"+sett.image_name)			
    ax.imshow(image)
    for gr in grid:
