...
if __name__=="__main__":
    ...
    parser.add_argument('-rt','--run_type',type=int,dest="run_type",default=0,help= """Type of run: 
        0 = standard, PSO_it = 1200*rf   PSO_prt = 400*rf    MCMCb = 1000*rf  MCMCr = 6000*rf 
        1 = append  MCMCb = 1000*rf (only used if no previous MCMC found )  MCMCr=6000*rf 
        2 = test run  PSO_it = 3   PSO_prt = 3   MCMCb = 1 MCMCr = 2 
        3 = append test   MCMCb = 2  MCMCr=2 
        4 = append PSO (TEST) read PSO and continue from there as initial point PSO_it = 1200*rf   PSO_prt = 400*rf  MCMCb = 2000*rf  MCMCr = 8000*rf"
    ...
    append_PSO=False
    if run_type==0:
        n_iterations = int(2500*run_fact) #number of iteration of the PSO run
        n_particles  = int(1000*run_fact) #number of particles in PSO run
        n_run  = int(6000*run_fact) #MCMC total steps 
        n_burn = int(1000*run_fact) #MCMC burn in steps
    elif run_type ==1:
        append_MC=True
        ...
        # PSO only
        n_iterations = int(1200*run_fact) #number of iteration of the PSO run
