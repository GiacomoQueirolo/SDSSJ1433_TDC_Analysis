...
if __name__=="__main__":
    ...
    parser.add_argument("-magB", "--mag_boundaries",dest="mag_boundaries", default=None,
    ...
    for sets in settings:
        ...
        if plot_prior:
            Rmag_ABC.append(np.mean(mag_prior_i.T[-1]))
            savefig_path = get_savefigpath(sets,backup_path)
            corner.corner(mag_prior_i,labels=labels,show_titles=True,truths=Rmag_ABC)
