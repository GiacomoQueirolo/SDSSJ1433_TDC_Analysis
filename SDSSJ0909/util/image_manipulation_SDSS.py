...
def match_image_sub(image,image_model,setting): 
    ...
    if setting.already_sub:
        ...
        try:
            dx = model_hdr["CRPIX1"] - header["CRPIX1"] + 1
            dy = model_hdr["CRPIX2"] - header["CRPIX2"] + 1
            # pragma no cover
            raise
        except:
...
def get_EE(setting,r):
    setting   = get_setting_module(setting).setting()
    data_path = setting.data_path
    data_path = "/".join(data_path.split("/")[:-1]) # main dir of all filters
...
def get_PHOTOFLAM(setting):
    ...
    if "F1"==filt[:2]:
        ...
        data_path = setting.data_path+"/"+setting.image_name
        return get_header(data_path,"PHOTOFLAM")
