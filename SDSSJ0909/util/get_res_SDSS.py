...
from util.input_data_SDSS import get_kwargs_model

def conv_mcmc_i_to_kwargs(setting,mcmc_i):
    setting = get_setting_module(setting).setting()    
    kwargs_model        = get_kwargs_model(setting)
    kwargs_lnslight_fix = setting.lens_light_params[2]
    kwargs_lns_fix      = setting.lens_params[2]
    kwargs_ps_fix       = setting.ps_params[2]
    if not check_if_WS(setting):
        kwargs_src_fix  = setting.source_params[2]
    else:
        kwargs_src_fix  = []
    param_class = Param(kwargs_model, kwargs_lns_fix, kwargs_src_fix,
                            kwargs_lnslight_fix, kwargs_ps_fix, [], [])
